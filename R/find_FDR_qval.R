#'@title find_FDR_qval
#'
#'@description Function that allows to retrieve FDR-qval value for a specific gene signature in GSEA results directory.
#'     
#'
...
#'
#'@param signature a R character element that contains the name of the signature for which the user wants an FDR-qval value for example "HALLMARK_E2F_TARGETS".
#'
#'@importFrom purrr keep
#'@importFrom rlang is_empty 
#'
#'@return A R named list. Each element of the R list corresponds to the FDR-qval value associated to a comparison between two conditions.
#'
#'@examples
...
#'  
#' find_FDR_qval(results_tsv,"KEGG_CELL_CYCLE")

find_FDR_qval <- function(gsea_report_list,signature){
#accolade ouvrante de find_FDR_qval
...
toret <- lapply(toret_step3,function(z){return(z[["FDR q-val"]])})

return(toret)


# accolade fermante de find_FDR_qval
}
